<template>
  <a-modal :open="isOpen" :footer="null" @cancel="$emit('cancel')">
    <AuthTemplate without-logo without-else class="auth-template">
      <p v-if="isMore" class="help-text help-text-red">
        Таңдалған {{ checkedCargoList.length }} жазбаға әсер етпек!
      </p>
      ...
      <a-form :model="cargoForm" layout="vertical" @finish="handleUpdateCargo">
        <a-form-item
          label="Зат атауы"
          name="name"
          :class="{ 'name-is-more': isMore }"
        >
          <a-input v-model:value="cargoForm.name" maxLength="30" />
          <span v-if="isMore" class="help-text"
            >Бос мәтін болғанда бәрінің атауы өзгермей қалады</span
          >
        </a-form-item>
        ...
        <a-form-item label="Статус" name="status" class="status-field">
          <ul class="status-list">
            <!-- @vue-skip -->
            <li
              v-for="(status, index) in cargoForm.statuses"
              :key="status.id ?? index"
              class="status"
            >
              <div class="left">
              ...
              <div class="right">
                ...
                <span v-if="status.time" class="time">{{
                  dateFormatter(status.time)
                }}</span>
              </div>
            </li>
          </ul>
        ...
        <a-form-item>
          <a-button
            type="primary"
            html-type="submit"
            :loading="loadingBatchUpdateCargo"
            >Өзгерісті сақтау</a-button
          >
        </a-form-item>
      </a-form>
    </AuthTemplate>
...
defineOptions({ name: "UpdateCargoModal" });
...
// 是否需要选择接收人
const selectUserBlockVisible = computed(
  () => cargoForm.value.statuses.at(-1)?.isSelected
);
...
watch(
  ...
  (newProps) => {
    if (newProps.length === 1) {
      ...
      cargoForm.value = {
        ...newProps[0],
        statuses: newProps[0].statusList.map((status) => ({
          ...status,
          isSelected: status.time !== null,
        })),
      };
    } else {
      cargoForm.value = {
...
const handleSelectStatus = (
  status: I_Cargo & { isSelected: boolean },
  index: number
) => {
...
const { runAsync: batchUpdateCargo, loading: loadingBatchUpdateCargo } =
  useRequest(API_CargoBatchOperate, { manual: true });
...
const handleUpdateCargo = () => {
  ...
  if (cargoForm.value.id) {
    ...
    if (typeof cargoForm.value.id === "number") {
      ...
      // cargoId + statusId []
      payload.cargoList = checkedCargoList.value.map(({ id, statusList }) => ({
        id,
        statusList: statusList.map(({ id }) => id),
      }));

      batchUpdateCargo(payload)
        .then(({ data }) => {
          cargoStore.database.cargoList = data.map((cargo) => ({
            ...cargo,
            checked: false,
          }));
    ...
    // 本地修改
      ...
      // @ts-ignore
      cargoStore.addedCargoListFromFile.cargoList =
        cargoStore.addedCargoListFromFile.cargoList.map((cargo) => {
          if (cargo.id === editedFields.id) {
            return {
              ...cargo,
              ...editedFields,
              statusList: cargo.statusList.map((status, index) => ({
                ...status,
  ...
  checkedCargoList.value.forEach(({ id }) => {
    cargoStore.addedCargoListFromFile.cargoList =
      cargoStore.addedCargoListFromFile.cargoList.map((cargoInFile) => {
